<template>
  ...
  <div ref="checkout" class="orderSummary" dir="ltr">
      ...
      <div class="d-flex flex-column gap-2">
        ...
        <span>
          ...
          <span class="ms-2">{{
            formatDate(
              new Date(
                new Date(bookInfo.created_at).getTime() + 4 * 60 * 60 * 1000
              )
            )
          }}</span>
        </span>
    ...
    <!-- <img src="https://firebasestorage.googleapis.com/v0/b/seasons-images.appspot.com/o/messages%2Fcars.jpeg?alt=media&token=b293faaa-a193-49cc-bc53-c28f59b01b06"
            style="display: block" alt="" height="250" width="768"> -->
...
// const download = () => {
//     axios({
//         url: 'https://firebasestorage.googleapis.com/v0/b/seasons-images.appspot.com/o/messages%2Fcars.jpeg?alt=media&token=b293faaa-a193-49cc-bc53-c28f59b01b06',
//         method: 'GET',
//         responseType: 'blob'
//     })
//         .then((response) => {
...
onMounted(async () => {
  if ((+route.params.id).toString() !== "NaN") {
    ...
    await axios
      .get(`https://seasonreal.seasonsge.com/appv1real/car-rr?id=${userId.id}`)
      .then((data) => {
        console.log(data, ";;;;;;;;;;;;;;;;;;;;;;;");
        ...
        axios
          ...
          .then(
            (data) =>
              (bookInfo.value.carType = data.data.filter(
                (el) => el.id == bookInfo.value.type_id
              )[0])
          );
      });
    url.value = window.location.href;
    url.value = url.value.replace(/\/\d+$/gi, `/${bookInfo.value.random_code}`);
    ...
    await axios
      ...
      .then((data) => {
        ...
        axios
          .get(
            `https://seasonreal.seasonsge.com/appv1real/user-data?user_id=${bookInfo.value.account_owner}`
          )
          .then((data) => {
