      ...
      <tbody>
        <tr>
          ...
          <td>
            {{
              stopsNums == 0
                ? "Direct/without stop places"
                : bookInfo.flight?.locStops
            }}
          </td>
          <td>
            {{
...
onMounted(async () => {
  if ((+route.params.id).toString() !== "NaN") {
    ...
    await axios
      .get(`https://seasonreal.seasonsge.com/appv1real/fli-rr?id=${userId.id}`)
      .then((data) => {
        console.log(data);
        ...
        axios
          .get(
            `https://seasonreal.seasonsge.com/appv1real/flights?flight_id=${bookInfo.value.flight_number}`
          )
          .then((data) => {
            console.log(data);
            ...
            axios
              .post(
                "https://seasonreal.seasonsge.com/appv1real/viewAirportById",
                from
              )
              .then((data) => {
    ...
    url.value = url.value.replace(/\/\d+$/gi, `/${bookInfo.value.booking_id}`);
  } else {
    url.value = window.location.href;
    await axios
      .get(
