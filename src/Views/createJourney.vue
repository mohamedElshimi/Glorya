                ...
                <div class="box overflow-hidden bg-white rounded-2 d-flex flex-column h-100">
                    <div class="image">
                        <router-link v-if="!item.link.includes('http')" :to="`/${$i18n.locale}/${item.link}`">
                            <img v-if="images[index]"  class="img-fluid h-100 w-100" :src="`https://seasonreal.seasonsge.com/appv1real/${images[index+1].image}`" alt="" />
                        </router-link>
                        <a v-if="item.link.includes('http')" target="_blank" :href="item.link">
                            <img v-if="images[index]" class="img-fluid w-100 h-100" :src="`https://seasonreal.seasonsge.com/appv1real/${images[index].image}`" alt="" />
                        </a>
                    </div>
