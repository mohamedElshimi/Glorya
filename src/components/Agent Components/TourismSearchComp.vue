<template>
    <div class="tourism-search p-4">
                ...
                <form class="d-flex gap-4 flex-column">
                    ...
                    <div class="d-flex gap-3 flex-md-row flex-column">
                        ...
                        <label class="w-100" v-if="searchInfo.includeFlight == '0'">
                            ...
                            <VueDatePicker v-model="searchInfo.toDate" :min-date="new Date().toLocaleDateString('en-CA')"
                    ...
                    <div class="bottom d-flex gap-3 flex-md-row flex-column">
                        <label class="w-100">
                            ...
                            <input class="rounded-1 p-2 px-3 w-100" type="number" min="1" v-model="searchInfo.no_adults">
                        </label>
                        <label class="w-100">
                            <span class="d-block mb-1 fw-semibold">{{ $t('programs.search.children') }}: </span>
                            <input class="rounded-1 p-2 px-3 w-100" type="number" min="0" v-model="searchInfo.no_children">
                        </label>
                        <label class="w-100">
                            <span class="d-block mb-1 fw-semibold">{{ $t('programs.search.infants') }}:</span>
                            <input class="rounded-1 p-2 px-3 w-100" type="number" min="0" v-model="searchInfo.no_infants">
                        </label>
                    </div>
                        ...
                        <table class="mt-4">
                            ...
                            <tbody>
                                <tr class="rooms"
                                    ...
                                    <td>
                                        <select class="rounded-1 p-2 px-3 w-100" name="room type" v-model="item.roomType">
                                    ...
                                    <td>
                                        <select class="rounded-1 p-2 px-3 w-100" name="room type"
                                            ...
                                            <option value="child with bed">{{ $t('programs.search.childWithBed') }}</option>
                                        </select>
                                    </td>
                                    <td>
        ...
        <TourismTable v-else-if="forward === '2'" :searchResults="searchResults" :searchInfo="searchInfo"
            @back-ward="() => { forward = '1', searchResults = [] }" @getIdAndForward="getIdAndForward"></TourismTable>
        <TourismBook v-else-if="forward === '3'" @back-ward="forward = '2'" :searchInfo="searchInfo"
            :searchResults="searchResults" :programId="programId" :cities="cities"></TourismBook>
...
import TourismTable from './TourismTable.vue';
import TourismBook from './TourismBook.vue';
...
onMounted(async () => {
    ...
    await axios.get("https://seasonreal.seasonsge.com/appv1real/country-view")
        ...
        for(let i = 0 ; i < allPrograms.value.length ; i++){
            if( allPrograms.value[i].image_1 !='' &&  allPrograms.value[i].image_1 !=null &&  allPrograms.value[i].image_2 !='' && allPrograms.value[i].image_2 !=null &&  allPrograms.value[i].image_3 !=''  && allPrograms.value[i].image_3 !=null && allPrograms.value[i].image_4 !='' &&  allPrograms.value[i].image_4 !=null && allPrograms.value[i].image_5 !='' && allPrograms.value[i].image_5 !=null){
           if(allPrograms.value[i].includes_flight == 1){
